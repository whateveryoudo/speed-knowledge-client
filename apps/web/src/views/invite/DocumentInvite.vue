    ...
    <a-card v-if="!linkLosed" :title="false" class="mt-10 mx-auto w-[432px]">
        <a-flex v-if="invitationValidInfo.invitation.need_approval === 1" vertical gap="10" align="center"
            ...
            <p class="text-[var(--sd-grey-9)] mb-4">
                申请加入文档协作
                <span class="text-[16px]">{{ invitationValidInfo.invitation.document_name }}</span>
            </p>
...
const options = computed(() => ({ token: route.query.token as string, teamSlug: route.params.team_slug as string, knowledgeSlug: route.params.knowledge_slug as string, documentSlug: route.params.document_slug as string | undefined, resourceType: route.params.resource_type as CollaboratorResourceType }));
