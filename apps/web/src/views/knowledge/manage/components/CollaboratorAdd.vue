<template>
    <s-full-modal class="collaborator-add-modal" :footer="false" :visible="visible"
        ...
        <template #title>
            <a-flex class="px-4 pt-2 border-solid border-0 border-b-1 border-b-[var(--sd-grey-2)]" align="center"
                justify="space-between">
                <ul class="flex gap-6 h-[40px] leading-[40px]">
        ...
        <div class="p-4" v-if="source === 1">
            <div v-if="!!secondModalType">
                <p class="text-[var(--sd-grey-8)]">{{ secondModalType === 'role' ? '可编辑，将拥有知识库的编辑权限。可阅读，仅拥有阅读和评论权限' :
                    '开启后，需经过手动审批确认，获得链接的人才能成为协作者。' }}</p>

                <a-flex vertical gap="2" v-if="secondModalType === 'role'">
                    <a-flex justify="space-between" align="center"
                        @click="handleUpdateTokenInfo({ 'role': KnowledgeCollaboratorRole.READ })">
                        <span>可阅读</span>
                        <check-outlined v-if="tokenInfo.role === KnowledgeCollaboratorRole.READ" fill="#25b864" />
                    </a-flex>
                    <a-flex justify="space-between" align="center"
                        @click="handleUpdateTokenInfo({ 'role': KnowledgeCollaboratorRole.EDIT })">
                        <span>可阅读</span>
                        <check-outlined v-if="tokenInfo.role === KnowledgeCollaboratorRole.EDIT" fill="#25b864" />
                    </a-flex>
                </a-flex>
                ...
                <a-flex vertical class="bg-[var(--sd-grey-1)] rounded-[8px] p-[6px] pt-[16px]">
                    <a-flex vertical>
                        <a-flex align="center" :gap="10" class="px-2 mb-2">
                            <a-input class="flex-1" disabled :value="inviteUrl" />
                            <a-button class="shrink-0" @click="handleCopy()">复制链接</a-button>
                        </a-flex>
                        <a-flex class="menu-item-base h-[40px]! hover:bg-[var(--sd-bg-primary-hover)]" align="center"
                            justify="space-between">
                        ...
                        <a-flex class="menu-item-base h-[40px]! hover:bg-[var(--sd-red-1)]" align="center"
                            justify="space-between">
...
import { type KnowledgeInvitationResponse, type UserInfo, KnowledgeInvitationStatus, KnowledgeCollaboratorRole } from '@sk/types';
