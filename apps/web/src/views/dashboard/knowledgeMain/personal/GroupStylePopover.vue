...
const layoutOptions = [
  { label: '卡片', value: KnowledgeGroupType.CARD, icon: AppstoreOutlined },
  { label: '列表', value: KnowledgeGroupType.LIST, icon: BarsOutlined },
]
    ...
    <template #content>
      <div class="w-[280px]">
        ...
        <div class="mb-4">
          ...
          <div class="grid grid-cols-2 gap-2">
            <button
              ...
              :key="item.value"
              type="button"
              class="flex flex-col items-center gap-1 rounded-md border border-solid px-3 py-2 text-[12px] transition-colors"
              :class="
                localConfig.type === item.value
                  ? 'border-[var(--sd-primary-color)] text-[var(--sd-primary-color)]'
                  : 'border-[var(--sd-border-light)] text-[var(--sd-grey-8)]'
              "
        ...
        <div v-if="localConfig.type === KnowledgeGroupType.CARD" class="mb-4">
          ...
          <div class="grid grid-cols-4 gap-2">
            <button
              ...
              :key="item.value"
              type="button"
              class="flex flex-col items-center gap-1 rounded-md border border-solid px-2 py-2 text-[11px] transition-colors"
              :class="
                localConfig.style === item.value
                  ? 'border-[var(--sd-primary-color)] text-[var(--sd-primary-color)]'
                  : 'border-[var(--sd-border-light)] text-[var(--sd-grey-8)]'
              "
        ...
        <div class="mb-3 flex items-center justify-between text-[13px]">
          <span>显示知识库图标</span>
          <a-switch
            v-model:checked="localConfig.show_knowledge_icon"
            size="small"
            @change="emitUpdate"
          />
        </div>
        <div class="mb-3 flex items-center justify-between text-[13px]">
          <span>显示知识库描述</span>
          <a-switch
            v-model:checked="localConfig.show_knowledge_description"
            size="small"
            @change="emitUpdate"
          />
        </div>
        ...
        <div class="flex items-center justify-between text-[13px]">
          ...
          <a-select
            size="small"
            class="w-[110px]"
            :value="localConfig.doc_order_type"
            :options="docOrderOptions"
