...
const columns: TableColumnType<KnowledgeItem>[] = [
  ...
  },
  {
    ...
    width: 200,
    customRender: ({ record }) => formatUpdateTime(record),
  },
  {
    title: '',
    key: 'action',
...
<template>
  <a-table row-key="id" :columns="columns" :data-source="dataSource" :loading="loading" :pagination="pagination"
    :scroll="{ x: 800 }">
    <template #bodyCell="{ column, record }">
      <template v-if="column.dataIndex === 'name'">
        <div class="flex min-w-0 cursor-pointer items-center gap-2" @click="goKnowledge(record as KnowledgeItem)">
          <s-icon-font :type="(record as KnowledgeItem).icon" svg-sprite class="shrink-0"
            style="width: 22px; height: 22px" />
          <a-input v-if="isRenaming((record as KnowledgeItem).id)" ref="renameInputRef" size="small"
            class="min-w-0 flex-1" :value="(record as KnowledgeItem).name" @click.stop
    ...
    <template #emptyText>
      <Empty0 has-top description="暂无知识库" />
    </template>
  </a-table>
